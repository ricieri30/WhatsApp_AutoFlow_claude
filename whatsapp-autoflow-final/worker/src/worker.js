...
new Worker("wa-scheduler", async (job) => {
  ...
  if (job.name === "pipeline-onboarding") {
    ...
    for (const step of cfg.steps.sort((a,b) => a.order - b.order)) {
      ...
      try {
        if (step.type === "text") {
          await sendText(pc.phoneE164, render(step.content, { nome: pc.name || "" }));
  ...
  if (job.name === "pipeline-weekly-check") {
    ...
    for (const weekCfg of (cfg.weeks || []).sort((a,b) => a.week - b.week)) {
      ...
      for (const pc of contacts) {
        ...
        try {
          const text = render(weekCfg.message, { nome: pc.name || "" });
    ...
    if (cfg.renewalMessage) {
      ...
      for (const pc of ending) {
        ...
        try {
          await sendText(pc.phoneE164, render(cfg.renewalMessage, { nome: pc.name || "" }));
  ...
  if (job.name === "check-subscriptions") {
    ...
    for (const { contacts, tpl } of groups)
      for (const c of contacts) {
        try { await sendText(c.phoneE164, render(tpl, { nome: c.name||"" })); total++; await sleep(delay()); } catch {}
      }
  ...
  for (const c of targets) {
    ...
    await sendText(c.phoneE164, render(tpl.body, { nome: c.name||"" }));
